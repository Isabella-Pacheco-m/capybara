...
export default function EventRegister() {
  ...
  const [registeredAccessCode, setRegisteredAccessCode] = useState('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Guardar el access code y mostrar modal
      setRegisteredAccessCode(result.access_code);
  ...
  const handleGoToDirectory = () => {
    // Redirigir al directorio (el usuario ingresará su código allí)
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-scale-up">
            <div className="text-center">
              ...
              <div className="bg-gray-50 rounded-2xl p-6 mb-6">
                <p className="text-sm text-gray-600 mb-3">Tu código de acceso es:</p>
                <div className="bg-white border-2 rounded-xl p-4 mb-4" style={{ borderColor: brandColors.primary }}>
                  <p className="text-2xl font-bold font-mono tracking-wider" style={{ color: brandColors.primary }}>
                    {registeredAccessCode}
                  </p>
                </div>
                <p className="text-sm text-gray-600">
                  También te hemos enviado este código a tu correo electrónico
                </p>
              </div>

              <button
                onClick={handleGoToDirectory}
                className="w-full text-white font-bold py-4 px-6 rounded-xl transition shadow-lg text-lg"
                style={{
                  background: `linear-gradient(135deg, ${brandColors.primary} 0%, ${brandColors.secondary} 100%)`
                }}
              >
